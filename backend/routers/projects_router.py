...
def get_db():
    ...
    This is obtained lazily so Firebase Admin can be initialized first.
    """
...
class ProjectCreate(BaseModel):
    ...
    topic: str
    # For docx: [{"title": "Section 1", "order": 0}]
    outline: Optional[List[Dict[str, Any]]] = None
    # For pptx: [{"title": "Slide 1", "order": 0}]
...
class ProjectResponse(BaseModel):
    ...
    # ✅ Include these so FastAPI does NOT strip them from responses
...
async def get_projects(decoded_token: dict = Depends(verify_token)):
    ...
    for doc in docs:
        ...
        projects.append({"id": doc.id, **project_data})
